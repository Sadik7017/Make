...
class ModuleMakeCommand extends Command
{
    ...
    protected $moduleNameSnake;       // snake_case name
    protected $moduleNameKebab;       // kebab-case name
    protected $moduleNamePascal;      // PascalCase name
    protected $moduleNameCamel;       // camelCase name
    protected $moduleNameUpper;       // UPPERCASE name
    protected $moduleNameLowerStudly;   // lowerStudlyCase name
    protected $moduleNameLowerSnake;    // lower_snake_case name
    protected $moduleNameLowerKebab;    // lower-kebab-case name
    ...
    public function handle()
    {
            ...
            $this->moduleNameStudly = Str::studly($this->moduleName);                   // e.g., SadikMalik     P 
            $this->moduleNamePascal = $this->moduleNameStudly;                          // Same as StudlyCase   P 
            $this->moduleNameCamel = Str::camel($this->moduleName);                     // e.g., sadikMalik     N
            $this->moduleNameSnake = Str::snake($this->moduleName);                     // e.g., sadik_malik    N
            $this->moduleNameKebab = Str::kebab($this->moduleName);                     // e.g., sadik-malik    N
            $this->moduleNameLower = strtolower(preg_replace('/[^a-zA-Z0-9]/', '', $this->moduleName)); // e.g., sadikmalik123   P
            $this->moduleNameUpper = strtoupper($this->moduleNameLower);               // e.g., SADIKMALIK             P

            // Custom: lowerStudlyCase = sadikMalik (like camelCase)
            $this->moduleNameLowerStudly = lcfirst(Str::studly($this->moduleName));     // e.g., sadikMalik    N

            // Custom: lower_snake_case
            $this->moduleNameLowerSnake = strtolower(Str::snake($this->moduleName));    // e.g., sadik_malik   N 

            // Custom: lower-kebab-case
            $this->moduleNameLowerKebab = strtolower(Str::kebab($this->moduleName));    // e.g., sadik-malik   N
    ...
    protected function createModuleDirectories()
{
    ...
    protected function createModuleFiles()
    {
        ...
        $repositoryStub = File::get(base_path('modules/Core/src/Console/Commands/stubs/repository.stub'));
        $repositoryStub = str_replace('{{ module_name }}', $this->moduleNameStudly, $repositoryStub);
        $repositoryStub = str_replace('{{ model_name }}', 'Base', $repositoryStub);
        ...
        $serviceStub = str_replace('{{ model_name }}', 'Base', $serviceStub);
