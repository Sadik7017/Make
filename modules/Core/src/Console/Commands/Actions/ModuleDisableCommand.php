    ...
    public function handle()
    {
        ...
        try {
            ...
            if (File::exists("{$modulePath}/src/Database/Migrations")) {
                ...
                $migrations = array_reverse($moduleState->applied_migrations ?? []);
                ...
                DB::table('module_states')
                    ...
                    ->update([
                        'status' => 'disabled',
                        'applied_migrations' => json_encode(array_diff($moduleState->applied_migrations ?? [], $rolledBackMigrations)),
                        'failed_migrations' => json_encode(array_merge($moduleState->failed_migrations ?? [], $failedRollbacks)),
                        'last_disabled_at' => now()
                    ]);
            }
